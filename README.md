...
As KrakN uses command line as a form of communication with user, the preferred way of executing software is by typing eg. `dataset_builder.exe` in CMD or `.\dataset_builder.exe` in PowerShell opened in the `*.exe` directory. This way you will be albe to read any error messages if they occur. However it is also possible to run KrakN software with double click.
